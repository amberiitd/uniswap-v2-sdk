...
import { ChainId, CurrencyAmount, ETHER, Pair, Percent, Route, Router, Token, TokenAmount, Trade, WRAPPED_NATIVE } from '../src'
...
describe('Router', () => {
  ...
  const pair_weth_0 = new Pair(new TokenAmount(WRAPPED_NATIVE[ChainId.MAINNET], '1000'), new TokenAmount(token0, '1000'))

  describe('#swapCallParameters', () => {
    describe('exact in', () => {
      it('ether to token1', () => {
        const result = Router.swapCallParameters(1, 
          Trade.exactIn(new Route([pair_weth_0, pair_0_1], ETHER, token1), CurrencyAmount.native(JSBI.BigInt(100), 1)),
        ...
        expect(result.args.slice(0, -1)).toEqual([
          '0x51',
          [WRAPPED_NATIVE[ChainId.MAINNET].address, token0.address, token1.address],
      ...
      it('token1 to ether', () => {
        ...
        expect(result.args.slice(0, -1)).toEqual([
          '0x64',
          '0x51',
          [token1.address, token0.address, WRAPPED_NATIVE[ChainId.MAINNET].address],
          '0x0000000000000000000000000000000000000004'
        ])
    ...
    describe('exact out', () => {
      ...
      it('token1 to ether', () => {
        const result = Router.swapCallParameters(1, 
          Trade.exactOut(new Route([pair_0_1, pair_weth_0], token1, ETHER), CurrencyAmount.native(JSBI.BigInt(100), 1)),
          { ttl: 50, recipient: '0x0000000000000000000000000000000000000004', allowedSlippage: new Percent('1', '100') }
        )
